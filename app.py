...
def interact_status(session_id):
    ...
    try:
        sess = InteractSession.query.get(session_id)
        if not sess:
        ...
        current_item = items[sess.current_index]

        if current_item['url']:
            return jsonify({
                'status': 'ready',
                'item': current_item,
                'item_number': sess.current_index + 1,
...
def interact_item_complete(session_id):
    ...
    try:
        ...
        sess.items = items  # trigger JSON re-serialisation
        ...
        return jsonify({
            'success': True,
            'next_item_index': sess.current_index,
            'total_items': len(items),
            'session_complete': sess.current_index >= len(items)
        })
